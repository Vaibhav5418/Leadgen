...
export default function Sidebar({ isOpen = false, onClose }) {
  ...
  const isAdminUser = 
    ...
    currentUser.email === 'akshay@kology.co';
    ...
    `}>
      ...
      <nav className="flex-1 px-4 py-4 overflow-y-auto">
        <div className="space-y-1">
          ...
          <div className="mt-2">
            <button
              onClick={() => handleNavigation('/employee-performance')}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${
            ...
            >
              <span className={isActive('/employee-performance') ? 'text-blue-600' : 'text-gray-500'}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
              </span>
              <span className={`text-sm font-medium ${isActive('/employee-performance') ? 'text-blue-600' : 'text-gray-700'}`}>
                Employee Performance
              </span>
            </button>
          </div>
          ...
          {isAdminUser && (
            <div className="mt-4 pt-3 border-t border-gray-100">
              <div className="px-3 mb-1.5 flex items-center justify-between">
                ...
                <span className="px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider bg-purple-100 text-purple-700 rounded">
                  Admin
                </span>
              </div>
              <button
                onClick={() => handleNavigation(adminPanelItem.path)}
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-colors ${
                  isActive(adminPanelItem.path)
                    ? 'bg-purple-50 text-purple-700 font-semibold'
                    : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900'
                }`}
              >
                <div className="flex items-center gap-3">
                  <span className={isActive(adminPanelItem.path) ? 'text-purple-600' : 'text-gray-500'}>
                    {adminPanelItem.icon}
                  </span>
                  <span className={`text-sm font-medium ${isActive(adminPanelItem.path) ? 'text-purple-700' : 'text-gray-700'}`}>
