...
def process_file(filepath, activity_type):
    ...
    # Regex to find getYears end
    # We look for `const getYears = () => { ... };`
    match = re.search(r'const getYears = \(\) => \{.*?\};\s*', content[state_start_idx:], re.DOTALL)
    
    if not match:
        print(f"Could not find getYears in {filepath}")
        return
...
process_file('leadgen-frontend/src/pages/EmailReport.jsx', 'email')
process_file('leadgen-frontend/src/pages/LinkedInReport.jsx', 'linkedin')
